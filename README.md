...
## MyBuddy Motion (Phase 13)

Composes several existing Animator animations into one longer sequence from `/motion` — never bakes to video/GIF (that would need a whole new encoding pipeline and works against the "never collapse to a flat raster" principle the rest of this platform follows). The creative decisions already happened in Vector/Animator; Motion's job is deterministic arrangement, not generation, so **v1 has no LLM step at all** — manual clip placement only (pick an animation, set a start offset + x/y position + z-order).

- **A `MotionProject` arranges `MotionClip`s, each pointing at an existing `AnimationDocument`** — same "compose, don't own the data" relationship Animator has with Vector. Deleting the referenced animation cascades to the clip.
- **The one genuinely new piece is timeline re-mapping**: each clip's keyframe times are re-expressed as a percentage of the *project's* total duration (not the clip's own animation duration), shifted by its start offset. A clip that only occupies part of a longer composition gets explicit synthesized `0%`/`100%` "hold" stops (per property) so it deterministically holds its first/last value before/after its active window, rather than depending on implicit CSS keyframe-range behavior across a multi-clip composed timeline.
- **Markup emission still reuses `vector_service._render_object` directly**, exactly like Animator does — `@keyframes` rule names and element ids are namespaced per clip (`clip-<clip-id>-obj-<object-id>`) since the same `VectorObject` could in principle appear in more than one clip, and each clip's objects are wrapped in a `<g transform="translate(x,y)">` for positioning.

## MyBuddy Creative Director (Phase 14)

The final Creative Platform sub-phase, from `/creative` — orchestrates the other four for cross-asset consistency. A `CreativeProject` groups assets from any of Vector, Animator, or Motion via a generic `CreativeAsset` membership table, and an optional `BrandKit` (colors + a font name) gets woven into Vector generation prompts so multiple assets in a project share a visual identity.

- **No new generation protocol**: orchestration = prepending a short brand-guidance paragraph to whatever prompt the user gives, then calling the exact same `vector_service.create_document_from_scene` that `POST /vector/documents` already uses (extracted into the service specifically so both call sites share it — verified by the full `test_vector.py` suite still passing unchanged after the extraction).
- **`CreativeAsset.asset_id` is deliberately not a database-level foreign key** — it points at one of three different tables (`vector_documents`, `animation_documents`, `motion_projects`) depending on `asset_type`, the standard tradeoff of a polymorphic-association design. Ownership and existence are checked at the application layer on every attach.
- **Manual attach/detach exists alongside brand-aware generation** — detaching only removes the project membership row, never the underlying asset (which may belong to other projects or stand alone) — the same "editable, not just AI-driven" pattern every other phase already follows.
- **Honest limitation**: `BrandKit.font_family` is informational only. `VectorObject`'s `TEXT` props have no `font_family` field today, so nothing enforces the chosen font in rendered SVG output — it's surfaced to the model as a stylistic hint, not an enforced constraint. Actually rendering a chosen font would mean extending Vector's object schema, which is out of scope here.
