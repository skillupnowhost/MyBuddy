#!/usr/bin/env python
"""Standalone video editing job runner (background removal / green screen).
...
Unlike generate.py, this uses rembg (CPU-fast, no GPU dependency) rather than diffusion — the
one video-track operation in this repo that actually completes on CPU-only hardware, given
...
def main() -> int:
    ...
    parser.add_argument("--operation", required=True, choices=["REMOVE_BACKGROUND"])
